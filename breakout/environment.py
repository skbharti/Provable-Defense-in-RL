...
class ObservationPool(object):
    ...
    def __init__(self, observation_pool):
        ...
        self.current_observation_index = 0      ### index at which a new observation will be added, the latest observation is at index -1
    ...
    def get_pooled_observations(self):
        states =  np.copy(self.observation_pool[:, :, self.__shift(list(range(self.pool_size)), self.current_observation_index)])
        return states

    def __shift(self, seq, n):
        ### shift a sequence to start at index n exp shift [0,1,2,3] by 2 -> [2,3,0,1]
        n = n % len(seq)
        return seq[n:]+seq[:n]
